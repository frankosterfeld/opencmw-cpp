...
template<DomainObject T, typename MatchFunctor=void>
...
public:
    bool operator()(std::string_view notified, std::string_view subscribed) const override {
        if constexpr (std::is_same_v<T, std::string_view>) {
            if constexpr (std::is_same_v<MatchFunctor, void>) {
                return notified == subscribed;
            } else {
            ...
            try {
                if constexpr (std::is_same_v<MatchFunctor, void>) {
                    return notifiedObj == subscribedObj;
                } else {
...
namespace detail {
struct TimingCtxMatches { bool operator()(const opencmw::TimingCtx &maybeMatching, const opencmw::TimingCtx &matched) const { return maybeMatching.matches(matched); } };
}
